// Function to generate an email template for incorrect email scenarios
export function generateTextEmailIncorrect(
    ...
    `;
    return emailText; // Return the complete email template
  }
  
  // Function to generate an email template for successful login
  export function generateTextEmailCorrect(
    subject: string,      // Subject line for the email
    name: string,         // Recipient's name
    email: string,        // Recipient's email address
    password?: string     // Optional password, if available
  ): string {
    const emailText = `
      <!DOCTYPE html>
      <html>
      ...
      <body>
          <div class="container">
              ...
              <div class="section">
                  <h2>Account Details</h2>
                  <p><strong>Email:</strong> ${email}</p>
                  <p><strong>Password:</strong> ${!password ? "In the moment there is not password" : password}</p>
              </div>
  ...
  // Function to generate an email template for password reset requests
  export function generateTextEmailForgotPassword(
    name: string,         // Recipient's name
    link: string,         // Link for resetting the password
    key: string           // Key for security purposes
  ): string {
    const emailText = `
      <!DOCTYPE html>
      <html>
